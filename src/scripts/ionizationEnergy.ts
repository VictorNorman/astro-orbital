...
export function populateIonEnergyTables() {
  ...
  const orbitalEnergies = energies$.get()[0].totalEnergies;
...
function handleNumElectronsChangedByUser(groundStateTotalEnergy: number) {
  ...
  const orbitalEnergies = totalOrbitalEnergy(selectedElem!.selectedElementInfo!.number, newOrbitals, dynamic23Matrix);
  // remove total energy from beginning of array.
  const totalOE = orbitalEnergies.shift();
